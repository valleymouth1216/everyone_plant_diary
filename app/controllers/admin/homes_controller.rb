  ...
  def search_date
    ...
    if params[:tag_ids]&.values&.include?("1")
      ...
      params[:tag_ids].each do |key, value|
        ...
        @search_diary_dates = []
        @diary_books.each do |diary_book|
        @search_diary_dates += diary_book.diary_dates.where(start_time: Time.zone.parse(@start_date).at_beginning_of_day...Time.zone.parse(@end_date))
      end
      @search_diary_dates.uniq!
        if @search_diary_dates == []
          @search_diary_dates = DiaryDate.joins(:diary_book).where(start_time: Time.zone.parse(@start_date).at_beginning_of_day...Time.zone.parse(@end_date)).page(params[:page]).per(10)
          flash[:notice] = "タグ検索したタグがありませんので、すべて表示します。"
        end
      @search_diary_dates = Kaminari.paginate_array(@search_diary_dates).page(params[:page]).per(10)
    else
      @search_diary_dates = DiaryDate.joins(:diary_book).where(start_time: Time.zone.parse(@start_date).at_beginning_of_day...Time.zone.parse(@end_date)).page(params[:page]).order("start_time DESC").per(10)
    end
